        ...
        if (!chave) {
          ...
          if (chave.token_uniq != "") {
            ...
            if (err) {
              ...
              db.collection("chaves").updateOne(
                id_chave,
                chave_uniq,
                function (err, rp) {
                  if (err) {
                    res.status(403).send({
                      success: false,
                      msg: "Erro ao atualizar as chaves",
                    });
...
*-------------------------------------------------------------------*/
router.get(
  "/memberinfo" /* , passport.authenticate('jwt', { session: false }) */,
  function (req, res) {
    var id_gerado = req.headers.authorization;
    if (id_gerado) {
      Chave.findOne(
        ...
        function (err, chave) {
          ...
          if (!chave) {
            return res.status(403).send({
              success: false,
              msg: "Authentication failed. Chave not found.",
            });
          } else {
                          ...
                          db.collection("users").updateOne(
                            ...
                            function (err, dbb) {
                              if (err) {
                                ...
                                res.json({
                                  success: true,
                                  msg: "Welcome in the member area " + dbb,
                                });
                              }
                            }
                          );
                  ...
                  var cont = db
                    ...
                    .count(function (err, cnt) {
                      ...
                      newInvestor.save(function (err) {
                        ...
                        res.json({
                          success: true,
                          msg: "Successful created new business.",
                        });
                      });
                      var myquery = { username: decoded.username };
                      var newvalues = {
...
*-------------------------------------------------------------------*/
router.post(
  "/investidor_coletivo" /* , passport.authenticate('jwt', { session: false }) */,
  function (req, res) {
    var id_gerado = req.headers.authorization;
    console.log(id_gerado);
    if (id_gerado) {
...
*-------------------------------------------------------------------*/
router.post(
  "/business" /* , passport.authenticate('jwt', { session: false }) */,
  function (req, res) {
    var id_gerado = req.headers.authorization;
    console.log(id_gerado);
    if (id_gerado) {
...
*-------------------------------------------------------------------*/
router.post(
  "/business_idea" /* , passport.authenticate('jwt', { session: false }) */,
  function (req, res) {
    var id_gerado = req.headers.authorization;
    console.log(id_gerado);
    if (id_gerado) {
                          ...
                          businessIdeia: [
                            {
                              ...
                              matriz_financeira_url:
                                req.body.url_matriz_financeira,
                      ...
                      request(
                        ...
                        function (error, response, body) {
                          if (!error && response.statusCode === 200) {
                            var isnif = body.nif_validation;
                            var loc =
                              body.records["509442013"]["geo"]["region"];
                            if (isnif === true) {
                              if (loc === distrito) {
                                var teste = {
                                  username: user.username,
                                  ...
                                  location:
                                    body.records["509442013"]["geo"]["region"],
                                  website:
                                    body.records["509442013"]["contacts"][
                                      "website"
                                    ],
                                  companyType:
                                    body.records["509442013"]["structure"][
                                      "nature"
                                    ],
                                  capitalSocial:
                                    body.records["509442013"]["structure"][
                                ...
                                res.json({
                                  success: false,
                                  msg: "O DISTRITO NAO CORRESPONDE",
                                });
                              }
                            } else {
                              console.log("O NIF NAO CORRESPONDE");
                                ...
                                if (loc === distrito) {
                                  var teste = {
                                    ...
                                    nomeEmpresa:
                                      body.records["509442013"]["title"],
                                    morada:
                                      body.records["509442013"]["address"],
                                    cp1: body.records["509442013"]["pc4"],
                                    cp2: body.records["509442013"]["pc3"],
                                    cae: body.records["509442013"]["cae"],
                                    location:
                                      body.records["509442013"]["geo"][
                                        "region"
                                      ],
                                    website:
                                      body.records["509442013"]["contacts"][
                            ...
                            function (err, product) {
                              ...
                              newContact.save(function (err, wishlist) {
                                if (err) {
                                  ...
                                  res.json({
                                    sucess: true,
                                    msg: "Adicionado com sucesso à wishlist",
                                  });
                                }
                              });
                            }
                  ...
                  db.collection("contacts").deleteOne(
                    ...
                    function (err, user) {
                      ...
                      if (!user) {
                        ...
                        res.json({
                          success: true,
                          msg: "Produto da wishlist apagado com sucesso",
                        });
                      }
                    }
                  );
                    ...
                    .exec(function (err, contacts) {
                      ...
                      if (!Contact) {
                        ...
                        if (cnt === 0) {
                          ...
                          var addContactsToArray = async.queue(function (
                            task,
                            callback
                          ) {
                            User.findOne(
                          ...
                          addContactsToArray.drain = function () {
                            Contact.find({
                              id_user: user.accountNumber,
                              estado_contato: "Pendente",
                            }).count(function (err, cnt_pendente) {
                              Contact.find({
                                id_user: user.accountNumber,
                                estado_contato: "Pendente",
                              })
                                .skip(perPage * page - perPage)
                                .limit(perPage)
                                .exec(function (err, contacts_pendente) {
                                  if (err) throw err;
                                  if (!Contact) {
                                    return res.status(403).send({
                                      success: false,
                                    ...
                                    if (cnt_pendente === 0) {
                                      ...
                                      var addContactsToArray = async.queue(
                                        function (task, callback) {
                                          User.findOne(
                                            {
                                              accountNumber:
                                                contacts_pendente[contact]
                                                  .id_contacto,
                                            },
                                            function (err, contact) {
                                              if (err) throw err;
          ...
          Investor.findOne(
            ...
            function (err, user) {
              ...
              if (!user) {
                ...
                res.json({
                  success: true,
                  msg: "Welcome in the member area " + user,
                });
              }
            }
          );
